"""
Test Thai Week 2 Lesson 2.3 Implementation
"""
import unittest

from runtime.education.thai_lesson_2_3 import (
    create_lesson_2_3,
    load_cv_syllables,
    generate_lesson_2_3_exercises,
    assess_lesson_2_3_mastery,
    update_knowledge_state_2_3,
    update_mastery_tracker_2_3,
)
from runtime.education.knowledge_state import KnowledgeState
from runtime.education.mastery_tracker import MasteryTracker


class TestLesson23Implementation(unittest.TestCase):
    
    def test_create_lesson_2_3(self):
        """Test lesson creation."""
        lesson = create_lesson_2_3()
        
        self.assertEqual(lesson.subject_id, "thai_language")
        self.assertEqual(lesson.level, 2)
        self.assertIn("พยางค์", lesson.title)
        self.assertGreater(len(lesson.objectives), 0)
    
    def test_load_cv_syllables(self):
        """Test loading CV syllables."""
        syllables = load_cv_syllables()
        
        self.assertGreater(len(syllables), 0)
        
        # Check structure
        for syll in syllables:
            self.assertIn('syllable', syll)
            self.assertIn('ipa', syll)
            self.assertIn('tone', syll)
            self.assertEqual(syll['structure'], 'CV')
    
    def test_syllable_parsing(self):
        """Test that all generated syllables are valid CV structure."""
        syllables = load_cv_syllables()
        
        for syll in syllables:
            # CV syllable should have consonant + vowel
            self.assertGreaterEqual(len(syll['syllable']), 2)
            self.assertEqual(syll['structure'], 'CV')
    
    def test_generate_exercises(self):
        """Test exercise generation."""
        exercises = generate_lesson_2_3_exercises()
        
        self.assertGreater(len(exercises), 0)
        
        # Should have exercises for each type
        ...
        tone_count = sum(1 for ex in exercises if 'Tone' in ex.question)
        
        self.assertGreater(ipa_count, 0)
        self.assertGreater(structure_count, 0)
        self.assertGreater(tone_count, 0)
    ...
    def test_assess_mastery_perfect(self):
        ...
        exercises = generate_lesson_2_3_exercises()[:15]
        responses = [(ex, ex.expected_answer) for ex in exercises]
        
        assessment = assess_lesson_2_3_mastery(responses)
    ...
    def test_assess_mastery_90_percent(self):
        """Test 90% threshold."""
        exercises = generate_lesson_2_3_exercises()[:10]
    ...
    def test_knowledge_state_update(self):
        ...
        knowledge_state = KnowledgeState("thai_week2_lesson3")
        ...
        update_knowledge_state_2_3(assessment, knowledge_state)
...
class TestLesson23Integration(unittest.TestCase):
    ...
    def test_complete_learning_flow(self):
        ...
        # 4. Simulate practice (85% accuracy)
        responses = []
        ...
        update_knowledge_state_2_3(assessment, knowledge_state)
        update_mastery_tracker_2_3(assessment, mastery_tracker, lesson.id)
        
        # 7. Verify integration
        self.assertGreater(knowledge_state.correct_count, 0)
        self.assertIn(lesson.id, mastery_tracker.records)
        # 85% should NOT be mastered (requires 90%)
