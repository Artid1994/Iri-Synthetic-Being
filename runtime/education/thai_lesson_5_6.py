"""
Thai Week 5 Lesson 5.6: Want/Desire
Want/desire pattern: Subject + อยาก + Verb. Example: อยาก กิน (want to eat).
"""
import json
...
def create_lesson_5_6() -> Lesson:
    """Create Lesson 5.6: Want/Desire."""
    lesson = Lesson(
        subject_id="thai_language",
        level=5,
        title="การแสดงความต้องการ (Want/Desire)",
        objectives=['Use อยาก for expressing desire', 'Form desire sentences: S + อยาก + VERB', 'Understand want/desire structures', 'Apply desire expressions in practical contexts'],
        content="Want/desire pattern: Subject + อยาก + Verb. Example: อยาก กิน (want to eat).",
        prerequisites=[],  # Requires lesson 5.5
    )
    return lesson


def load_desire_examples() -> List[Dict]:
    """
    Load examples for Want/Desire.
    ...
    for pattern in grammar_data['conversational_patterns']:
        if pattern['pattern'] == 'want/desire':
            examples.append({
                'question': f"Meaning: {pattern['example']}",
                'answer': pattern['meaning'],
            })
            examples.append({
                'question': 'Want/desire structure',
...
def generate_lesson_5_6_exercises() -> List[LearningExercise]:
    """
    Generate practice exercises for Lesson 5.6.
    """
    examples = load_desire_examples()
    exercises = []
...
def assess_lesson_5_6_mastery(responses: List[Tuple[LearningExercise, str]]) -> Dict:
    """
    Assess mastery of Lesson 5.6.
    ...
    if len(error_patterns['errors']) > 2:
        knowledge_gaps.append("want/desire")
...
def update_knowledge_state_5_6(assessment: Dict, knowledge_state: KnowledgeState) -> None:
    """Update KnowledgeState based on Lesson 5.6 assessment."""
...
def update_mastery_tracker_5_6(assessment: Dict, mastery_tracker: MasteryTracker, lesson_id: str) -> None:
    """Update MasteryTracker based on Lesson 5.6 assessment."""
